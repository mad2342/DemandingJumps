        ...
        public static class AbstractActor_OnMovePhaseComplete_Patch
        {
            static void Postfix(AbstractActor __instance)
            {
                try
                {
                    ...
                    {
                        //Logger.LogLine("[AbstractActor_OnMovePhaseComplete_POSTFIX] __instance.Combat.LocalPlayerTeam.IsActive: " + __instance.Combat.LocalPlayerTeam.IsActive);

                        Fields.JumpPreview = false;
                        Logger.LogLine("[AbstractActor_OnMovePhaseComplete_POSTFIX] Fields.JumpPreview: " + Fields.JumpPreview);
                    }
                }
                catch (Exception e)
                {
        ...
        public static class CombatSelectionHandler_TrySelectActor_Patch
        {
            ...
            static void Postfix(CombatSelectionHandler __instance, AbstractActor actor)
            {
                try
                {
                    ...
                    //{
                        Fields.JumpPreview = false;
                        Logger.LogLine("[CombatSelectionHandler_TrySelectActor_POSTFIX] Fields.JumpPreview: " + Fields.JumpPreview);
                    //}
                }
                catch (Exception e)
                {
        ...
        public static class SelectionState_GetNewSelectionStateByType_Patch
        {
            ...
            static void Postfix(SelectionState __instance, SelectionType type, AbstractActor actor)
            {
                try
                {
                    ...
                    Logger.LogLine("[SelectionState_GetNewSelectionStateByType_POSTFIX] Fields.JumpPreview: " + Fields.JumpPreview);
                }
                catch (Exception e)
                {
                    Logger.LogError(e);
        ...
        */
        ...
        public static class CombatSelectionHandler_addNewState_Patch
        {
            ...
            static void Postfix(CombatSelectionHandler __instance, SelectionState newState)
            {
                try
                {
                    //Logger.LogLine("[CombatSelectionHandler_addNewState_POSTFIX] newState.SelectionType: " + newState.SelectionType);
                    Fields.JumpPreview = newState.SelectionType == SelectionType.Jump ? true : false;
                    Logger.LogLine("[CombatSelectionHandler_addNewState_POSTFIX] Fields.JumpPreview: " + Fields.JumpPreview);
                }
                catch (Exception e)
                {
                    Logger.LogError(e);
        ...
        public static class CombatSelectionHandler_RemoveTopState_Patch
        {
            ...
            static void Prefix(CombatSelectionHandler __instance)
            {
                try
                {
                    //Logger.LogLine("[CombatSelectionHandler_RemoveTopState_PREFIX] __instance.ActiveState.SelectionType: " + __instance.ActiveState.SelectionType);
                    if (__instance.ActiveState.SelectionType == SelectionType.Jump)
                    {
                        Fields.JumpPreview = false;
                    }
                    Logger.LogLine("[CombatSelectionHandler_RemoveTopState_PREFIX] Fields.JumpPreview: " + Fields.JumpPreview);
                }
                catch (Exception e)
                {
                    Logger.LogError(e);
        ...
        public static class ToHit_GetAllModifiers_Patch
        {
            public static void Postfix(ToHit __instance, ref float __result, CombatGameState ___combat, AbstractActor attacker, Weapon weapon, ICombatant target)
            {
                try
                {
                    ...
                    //Logger.LogLine("[ToHit_GetAllModifiers_POSTFIX] CombatGameState.LocalPlayerTeam.IsActive: " + ___combat.LocalPlayerTeam.IsActive);



                    bool AttackerJumpedThisRound = attacker.HasMovedThisRound && attacker.JumpedLastRound;
                    //Logger.LogLine("[ToHit_GetAllModifiers_POSTFIX] Fields.JumpPreview: " + Fields.JumpPreview);
                    //Logger.LogLine("[ToHit_GetAllModifiers_POSTFIX] AttackerJumpedThisRound: " + AttackerJumpedThisRound);
                    ...
                    if (Fields.JumpPreview || AttackerJumpedThisRound)
                    {
                        ...
                        //Logger.LogLine("[ToHit_GetAllModifiers_POSTFIX] Unit previews jump or already jumped. Applying ToHit penalty.");
        ...
        public static class ToHit_GetAllModifiersDescription_Patch
        {
            public static void Postfix(ToHit __instance, ref string __result, CombatGameState ___combat, AbstractActor attacker, Weapon weapon, ICombatant target)
            {
                try
                {
                    ...
                    //Logger.LogLine("[ToHit_GetAllModifiersDescription_POSTFIX] CombatGameState.LocalPlayerTeam.IsActive: " + ___combat.LocalPlayerTeam.IsActive);



                    bool AttackerJumpedThisRound = attacker.HasMovedThisRound && attacker.JumpedLastRound;
                    //Logger.LogLine("[ToHit_GetAllModifiersDescription_POSTFIX] Fields.JumpPreview: " + Fields.JumpPreview);
                    //Logger.LogLine("[ToHit_GetAllModifiersDescription_POSTFIX] AttackerJumpedThisRound: " + AttackerJumpedThisRound);

                    if (AttackerJumpedThisRound)
                    {
                        int ToHitSelfJumpedModifier = Utilities.GetAttackerJumpedAccuracyModifier(attacker);
                        Logger.LogLine("[ToHit_GetAllModifiersDescription_POSTFIX] Add description for ToHitSelfJumped: " + ToHitSelfJumpedModifier);
        ...
        public static class CombatHUDWeaponSlot_SetHitChance_Patch
        {
            public static void Postfix(CombatHUDWeaponSlot __instance, CombatGameState ___Combat, ICombatant target)
            {
                try
                {
                    ...
                    //Logger.LogLine("[CombatHUDWeaponSlot_SetHitChance_POSTFIX] CombatGameState.LocalPlayerTeam.IsActive: " + ___Combat.LocalPlayerTeam.IsActive);



                    AbstractActor attacker = __instance.DisplayedWeapon.parent;
                    bool AttackerJumpedThisRound = attacker.HasMovedThisRound && attacker.JumpedLastRound;
                    //Logger.LogLine("[CombatHUDWeaponSlot_SetHitChance_POSTFIX] Fields.JumpPreview: " + Fields.JumpPreview);
                    //Logger.LogLine("[CombatHUDWeaponSlot_SetHitChance_POSTFIX] AttackerJumpedThisRound: " + AttackerJumpedThisRound);
